                ...
                <div className="flex items-center gap-2">
                  {tweet.author.profile_image_url ? (
                    ...
                    <div className="w-10 h-10 rounded-full bg-zinc-700 flex items-center justify-center text-zinc-400">
                      <span className="text-lg">{tweet.author.name[0]}</span>
                    </div>
                  )}
                  <div>
                    ...
                    <div className="text-sm text-zinc-500">@{tweet.author.username}</div>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-6 pt-1">
                <div className="flex items-center gap-2 text-zinc-500">
                  ...
                  <span className="text-sm">
                    {tweet.public_metrics.reply_count > 0 ? tweet.public_metrics.reply_count.toLocaleString() : ''}
                  </span>
                </div>
                <div className="flex items-center gap-2 text-zinc-500">
                  <Repeat className="w-4 h-4" />
                  <span className="text-sm">
                    {tweet.public_metrics.retweet_count > 0 ? tweet.public_metrics.retweet_count.toLocaleString() : ''}
                  </span>
                </div>
                <div className="flex items-center gap-2 text-zinc-500">
                  <Heart className="w-4 h-4" />
                  <span className="text-sm">
                    {tweet.public_metrics.like_count > 0 ? tweet.public_metrics.like_count.toLocaleString() : ''}
                  </span>
                </div>
              </div>
            </div>
